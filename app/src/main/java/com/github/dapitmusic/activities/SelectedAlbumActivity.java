...
import com.github.dapitmusic.model.Album;
...
public class SelectedAlbumActivity extends AppCompatActivity {
	...
	private void setAlbumDataToUi() {
		...
		albumDetails.setText(String.format(Locale.getDefault(), "%s . %s . %d songs", album.music.get(0).artist,
				album.year, album.music.size()));

		boolean state = MPPreferences.getAlbumRequest(this);
		if (state)
			Glide.with(this).load(album.music.get(0).albumArt).placeholder(R.drawable.ic_notif_music_note)
					.into(albumArt);
	}
}